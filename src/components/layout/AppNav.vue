    ...
    <div class="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
      ...
      <div class="flex items-center gap-0.5 sm:gap-1">
        <RouterLink
          v-if="auth.isAdmin"
          to="/admin"
          class="flex items-center gap-1.5 px-2 sm:px-3 py-2 rounded-lg text-xs font-semibold uppercase tracking-wider transition-colors"
          :class="$route.path.startsWith('/admin') ? 'text-red-400 bg-red-500/10' : 'text-gray-400 hover:text-white'"
        >
          <Settings :size="14" /> <span class="hidden sm:inline">Admin</span>
        </RouterLink>
        <RouterLink
          v-else
          to="/"
        ...
        >
          <ClipboardList :size="14" /> <span class="hidden sm:inline">Check-in</span>
        </RouterLink>

        <!-- User + logout -->
        <div class="flex items-center gap-2 ml-1 sm:ml-2 pl-1 sm:pl-2 border-l border-white/10">
