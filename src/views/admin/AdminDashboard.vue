  ...
  <AdminLayout>
    <div class="admin-content">
      ...
      <div class="mb-10">
        ...
        <p class="text-sm text-zinc-500 mt-1.5">Privire de ansamblu asupra platformei</p>
      </div>

      <!-- Stats -->
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
        <button @click="router.push('/admin/clienti')"
          class="group relative bg-[#111113] border border-zinc-800 rounded-xl p-6 text-left hover:border-zinc-700 transition-all duration-200">
      ...
      <div>
        ...
        <div class="bg-[#111113] border border-zinc-800 rounded-xl overflow-hidden">
          ...
          <div v-if="loading" class="divide-y divide-zinc-800/50">
            <div v-for="i in 4" :key="i" class="h-[72px] animate-pulse bg-zinc-900/50"></div>
          </div>
          ...
          <div v-else class="divide-y divide-zinc-800/50">
            <div v-for="ci in recentCheckins" :key="ci.id"
              class="flex items-center gap-4 px-5 py-4 hover:bg-zinc-800/20 transition-colors">
              ...
              <!-- Date -->
              <span class="text-xs text-zinc-500 flex-shrink-0">{{ formatDate(ci.created_at) }}</span>
            </div>
          </div>
        </div>
      </div>
